...
func dissAssemble(b []byte) Instr {
	m := lookUp(b)
	inst := Instr{label: m.label, family: m.family}
	switch m.label {
	case INSN_NOP:
		//fmt.Printf("%.4x\tnop\n", b2u16big(b))
		return inst
	...
	case INSN_LDI:
		...
		//fmt.Printf("%.4x\tldi\tr%d, 0x%.2x\t\t;%d\n", b2u16big(b), inst.dest, inst.kdata, inst.dest)
		return inst
	case INSN_RCALL:
		// 1101 kkkk kkkk kkkk
		k := (uint32(b[1]&0x0f)<<8 | uint32(b[0]))
	...
	case INSN_CPI:
		...
		inst.kdata = ((b[1] & 0x0f) << 4) | (b[0] & 0x0f)
		inst.dest = ((b[0] & 0xf0) >> 4) + 0x10
		//fmt.Printf("%.4x\tcpi\tr%d, 0x%.2x\n", b2u16big(b), inst.kdata, inst.dest)
		return inst
	case INSN_CPSE:
		// 0001 00rd dddd rrrr
		inst.source = (((b[1]&0x02)>>1)<<4 | (b[0] & 0x0f))
	...
	case INSN_LPMZ:
		...
		s := b[0] & 0x0f
		// XXX ToDo not tested
		// XXX ToDo(Erin) Not sure this works.inst.dest = (b2u16little(b) & 0x01f0) >> 4
		inst.dest = ((b[1] & 0x01) << 4) | ((b[0] & 0xf0) >> 4)
		if s == 6 {
